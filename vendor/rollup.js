		...
		return ( resolvedId ? Promise.resolve( resolvedId ) : this$1.resolveId( source, module.id ) )
			.then( function (resolvedId) {
				...
				if ( !resolvedId && !isExternal ) {
					...
					this$1.warn({
						code: 'UNRESOLVED_IMPORT',
						message: ("'" + source + "' is imported by " + (relativeId( module.id )) + ", but could not be resolved â€“ treating it as an external dependency"),
...
Bundle$$1.prototype.render = function render ( options ) {
	...
	if ( options.format === 'es6' ) {
		this.warn({
			code: 'DEPRECATED_ES6',
			message: 'The es6 format is deprecated â€“ use `es` instead'
		});
